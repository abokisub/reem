<?php
// Fix PointWave account conflicts where API returns existing account numbers
// This addresses the issue where PointWave API returns accounts already assigned to other users
...
if ($confirm !== 'CONFIRM') {
    echo "⚠️  WARNING: This will fix PointWave account conflicts\n";
...
echo "=== FIXING POINTWAVE ACCOUNT CONFLICTS ===\n";
echo "Phone: $phone\n\n";
...
foreach ($users as $user) {
    ...
    echo "  PointWave Account: " . ($user->pointwave_account_number ?? 'None') . "\n";
    echo "  PointWave Customer ID: " . ($user->pointwave_customer_id ?? 'None') . "\n";
    
    if ($user->pointwave_account_number) {
        // Check if this account number is used by other users
        $conflictingUsers = User::where('pointwave_account_number', $user->pointwave_account_number)
            ->where('id', '!=', $user->id)
            ->get();
            
        if ($conflictingUsers->count() > 0) {
            echo "  🚨 CONFLICT DETECTED: Account {$user->pointwave_account_number} is also assigned to:\n";
            ...
            echo "  Clearing PointWave account from user {$user->username}...\n";
            $user->update([
...
echo "2. CHECKING FOR DUPLICATE ACCOUNT NUMBERS:\n";
$duplicateAccounts = DB::table('users')
    ->select('pointwave_account_number', DB::raw('COUNT(*) as count'))
    ->whereNotNull('pointwave_account_number')
    ->groupBy('pointwave_account_number')
    ->having('count', '>', 1)
    ->get();

echo "Found " . $duplicateAccounts->count() . " duplicate account numbers:\n";
foreach ($duplicateAccounts as $duplicate) {
    echo "- Account {$duplicate->pointwave_account_number} assigned to {$duplicate->count} users\n";
    
    $usersWithAccount = User::where('pointwave_account_number', $duplicate->pointwave_account_number)->get();
...
echo "2. PointWave API should create new unique accounts\n";

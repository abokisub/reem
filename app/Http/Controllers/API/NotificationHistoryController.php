            ...
            if (!empty($request->id)) {
                ...
                if ($check_user->count() > 0) {
                    ...
                    if ($broadcast->target_type == 'ALL') {
                        ...
                        $all_user = DB::table('users')->where('username', $broadcast->target_username)->get();
                    } else {
                        $all_user = DB::table('users')->where('type', $broadcast->target_type)->get();
                    }
                    ...
                    foreach ($all_user as $user) {
                        ...
                        $change_habukhan = [$user->username, $user->email, $user->name, $user->phone, $user->webhook, $user->apikey, $user->address, $user->ref, $user->type, ($user->palmpay_account_number ?? 'N/A'), $user->rolex, $moniepoint_acc, $user->fed, $user->otp, $user->user_limit, '₦' . number_format($user->balance, 2), '₦' . number_format($user->referral_balance, 2)];

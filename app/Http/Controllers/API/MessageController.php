            ...
            if (!empty($request->id)) {
                ...
                if ($check_user->count() > 0) {
                    ...
                    if ($request->status == 'ALL') {
                        ...
                        $all_user = DB::table('users')->where('username', $request->user_username)->get();
                    } else {
                        $all_user = DB::table('users')->where('type', $request->status)->get();
                    }
                    foreach ($all_user as $user) {
                        ...
                        $moniepoint_acc = $palmpay_acc ?? 'Generating...';
                        $change_habukhan = [$user->username, $user->email, $user->name, $user->phone, $user->webhook, ($user->api_key ?? 'N/A'), $user->address, $user->ref, $user->type, ($palmpay_acc ?? 'N/A'), ($palmpay_acc ?? 'N/A'), $moniepoint_acc, ($palmpay_acc ?? 'N/A'), $user->otp, $user->user_limit, '₦' . number_format($user->balance, 2), '₦' . number_format($user->referral_balance, 2)];
        ...
        if (!$request->headers->get('origin') || in_array($request->headers->get('origin'), $explode_url)) {
            if (!empty($request->id)) {
                ...
                if ($check_user->count() > 0) {
                    ...
                    foreach ($all_user as $user) {
                        $moniepoint_acc = $user->palmpay_account_number ?? 'Generating...';
                        $change_habukhan = [$user->username, $user->email, $user->name, $user->phone, $user->webhook, ($user->api_key ?? 'N/A'), $user->address, $user->ref, $user->type, ($user->palmpay_account_number ?? 'N/A'), ($user->palmpay_account_number ?? 'N/A'), $moniepoint_acc, ($user->palmpay_account_number ?? 'N/A'), $user->otp, $user->user_limit, '₦' . number_format($user->balance, 2), '₦' . number_format($user->referral_balance, 2)];
                        $real_message = str_replace($habukhan_search, $change_habukhan, $request->message);
                    ...
                    if (!empty($tokens)) {
                        try {
                            ...
                            $firebase->sendMulticastNotification(
                                $tokens,
                                "New Update from " . config('app.name'),

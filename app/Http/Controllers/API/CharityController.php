...
class CharityController extends Controller
{
    ...
    public function searchUsers(Request $request)
    {
        ...
        if (!$request->headers->get('origin') || in_array($request->headers->get('origin'), $explode_url)) {
            ...
            if ($user_id) {
                $check_admin = DB::table('users')->where(['id' => $user_id, 'type' => 'ADMIN'])->first();
                if ($check_admin) {
        ...
        if (!$request->headers->get('origin') || in_array($request->headers->get('origin'), $explode_url)) {
            ...
            if ($user_id) {
                ...
                if ($check_admin) {
                    ...
                    $target_user = DB::table('users')->where('username', $request->username)->first();
        ...
        if (!$request->headers->get('origin') || in_array($request->headers->get('origin'), $explode_url)) {
            ...
            if ($user_id) {
                ...
                if ($check_admin) {
                    ...
                    $charity = DB::table('charities')->where('id', $request->charity_id)->first();
                    $target_user = DB::table('users')->where('id', $charity->user_id)->first();
    ...
    public function donate(Request $request)
    {
        ...
        if ($user_id) {
            ...
            DB::table('users')->where('id', $user_id)->decrement('balance', $amount);
    ...
    public function processPayouts(Request $request)
    {
        ...
        if (!$request->headers->get('origin') || in_array($request->headers->get('origin'), $explode_url)) {
            ...
            if ($user_id) {
                ...
                $closed_payouts = DB::table('campaigns')
                    ->where('status', 'closed')
                    ->where('payout_status', 'pending')
                    ->get();

                $count = 0;
                foreach ($closed_payouts as $campaign) {
                    // Start Transaction
                    DB::beginTransaction();
                    try {
                        // 1. Mark campaign as released
                        DB::table('campaigns')->where('id', $campaign->id)->update(['payout_status' => 'released']);
                        ...
                        DB::table('charities')->where('id', $campaign->charity_id)->increment('available_balance', $campaign->current_amount);

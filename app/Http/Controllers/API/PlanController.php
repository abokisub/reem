...
class PlanController extends Controller
{
            ...
            if (!empty($request->id)) {
                ...
                if ($check_user->count() == 1) {
                    ...
                    if ($main_validator->fails()) {
                        ...
                        $cid = $adex->active_company_id ?? 1;

                        $plans = DB::table('data_plan')
                            ->where(['network' => $get_network->network, 'plan_type' => $request->network_type, 'plan_status' => 1])
                            ->whereIn('company_id', [$cid, 1])
                            ->get()
                            ->groupBy('plan_id')
                            ->map(function ($group) use ($cid) {
                                return $group->where('company_id', $cid)->first() ?? $group->where('company_id', 1)->first();
                            });
    ...
    public function CablePlan(Request $request)
    {
        ...
        if (!$request->headers->get('origin') || in_array($request->headers->get('origin'), $explode_url)) {
            if (isset($request->id)) {
                ...
                $user = DB::table('users')->where('id', $this->verifytoken($request->id_token ?? $request->id))->first();
                $cid = $user->active_company_id ?? 1;

                $plans = DB::table('cable_plan')
                    ->where(['cable_name' => $cable_name->cable_name, 'plan_status' => 1])
                    ->whereIn('company_id', [$cid, 1])
                    ->get()
                    ->groupBy('plan_id')
                    ->map(function ($group) use ($cid) {
        ...
        if (!$request->headers->get('origin') || in_array($request->headers->get('origin'), $explode_url)) {
            if (isset($request->id)) {
                if (DB::table('cable_plan')->where('plan_id', $request->id)->count() == 1) {
                    ...
                    if ($cable_setting && $cable_setting->direct == 1) {
    ...
    public function ExamList(Request $request)
    {
        ...
        if (!$request->headers->get('origin') || in_array($request->headers->get('origin'), $explode_url)) {
            ...
            $exam_price = DB::table('result_charge')->where(['company_id' => $cid])->first();
            if (!$exam_price) {
                $exam_price = DB::table('result_charge')->where(['company_id' => 1])->first();
            }
    ...
    public function HomeData(Request $request)
    {
        ...
        if (!$request->headers->get('origin') || in_array($request->headers->get('origin'), $explode_url)) {
            return response()->json([
                'status' => 'success',
                'mtn' => DB::table('data_plan')->where(['network' => 'MTN', 'plan_status' => 1, 'company_id' => 1])->select('plan_name', 'network', 'plan_size', 'plan_day', 'smart')->orderBy('smart', 'asc')->get(),
                'glo' => DB::table('data_plan')->where(['network' => 'GLO', 'plan_status' => 1, 'company_id' => 1])->select('plan_name', 'network', 'plan_size', 'plan_day', 'smart')->orderBy('smart', 'asc')->get(),
                'airtel' => DB::table('data_plan')->where(['network' => 'AIRTEL', 'plan_status' => 1, 'company_id' => 1])->select('plan_name', 'network', 'plan_size', 'plan_day', 'smart')->orderBy('smart', 'asc')->get(),
                'mobile' => DB::table('data_plan')->where(['network' => '9MOBILE', 'plan_status' => 1, 'company_id' => 1])->select('plan_name', 'network', 'plan_size', 'plan_day', 'smart')->orderBy('smart', 'asc')->get()
            ]);
        } else {
            return redirect(config('app.error_500'));
            return response()->json([

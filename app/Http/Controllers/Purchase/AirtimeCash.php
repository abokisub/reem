                ...
                if ($user_check->count() == 1) {
                    ...
                    if (!$company_blocked) {
                        ...
                        if (DB::table('cash')->where('transid', $transid)->count() == 0 and DB::table('message')->where('transid', $transid)->count() == 0) {
                            ...
                            // check network lock
                            $net_lock = DB::table('network')->where(['network' => $request->network, 'company_id' => $cid])->first();
                            if (!$net_lock) {
                                $net_lock = DB::table('network')->where(['network' => $request->network, 'company_id' => 1])->first();
                            }
                                ...
                                if ($habukhan_bypass == true) {
                                    ...
                                    if ($habukhan_new_go) {
                                        if ((strtolower($request->payment_type) == 'wallet') xor strtolower($request->payment_type) == 'bank') {
                                            ...
                                            if (!empty($user->palmpay_account_number) || $user->type != 'WITHDRAW') {
                                                $system_admin = DB::table('cash_discount')->where('company_id', $cid)->first();
                                                if (!$system_admin) {
                                                    $system_admin = DB::table('cash_discount')->where('company_id', 1)->first();
                                                }
                                                ...
                                                if ($this->inserting_data('message', $trans_history) and $this->inserting_data('cash', $trans_cash)) {
                                                    ...
                                                    DB::table('request')->insert(['username' => $user->username, 'message' => $send_message, 'date' => $this->system_date(), 'transid' => $transid, 'status' => 'pending', 'title' => 'AIRTIME 2 CASH']);
                                                    return response()->json([
                                                        'status' => 'success',

    ...
    public function Buy(Request $request)
    {
        ...
        if (!$request->headers->get('origin') || in_array($request->headers->get('origin'), $explode_url)) {
            ...
            if ($this->core(1)->allow_pin == 1) {
                ...
                $check = DB::table('users')->where(['id' => $this->verifytoken($request->token), 'pin' => $request->pin]);
                if ($check->count() == 1) {
                    $det = $check->first();
        ...
        if ($accessToken) {
            ...
            if ($user_check->count() == 1) {
                ...
                if (DB::table('bulksms')->where('transid', $transid)->count() == 0 and DB::table('message')->where('transid', $transid)->count() == 0) {
                    $limit = $this->core($user->active_company_id ?? 1)->bulk_length;
                    if ($this->core($user->active_company_id ?? 1)->bulksms == 1) {
                        if ($validator->fails()) {
                            return response()->json([
                            ...
                            if ($habukhan_new_go == true) {
                                ...
                                if (count($number) <= 10000) {
                                    ...
                                    if ($real_number != null) {
                                        $charges = count($real_number) * $this->core($user->active_company_id ?? 1)->bulk_sms;
                                        DB::beginTransaction();
                                        $user = DB::table('users')->where(['id' => $user->id])->lockForUpdate()->first();
                                        if ($user->balance > 0) {
                                            if ($user->balance >= $charges) {
                                                $debit = $user->balance - $charges;
                                                ...
                                                if (DB::table('users')->where(['id' => $user->id])->update(['balance' => $debit])) {
                                                    ...
                                                    if ($this->inserting_data('message', $trans_history) and $this->inserting_data('bulksms', $bulk_history)) {
                                                        ...
                                                        if ($response == 'success') {
                                                            ...
                                                            DB::table('users')->where(['username' => $user->username, 'id' => $user->id]);

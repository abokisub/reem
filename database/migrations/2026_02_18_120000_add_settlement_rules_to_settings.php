...
return new class extends Migration
{
    public function up(): void
    {
        Schema::table('settings', function (Blueprint $table) {
            // Settlement configuration - check if columns exist before adding
        ...
        Schema::table('companies', function (Blueprint $table) {
            // Per-company settlement overrides - check if columns exist before adding
            if (!Schema::hasColumn('companies', 'custom_settlement_enabled')) {
                $table->boolean('custom_settlement_enabled')->default(false)->after('palmpay_bank_code');
            }
            if (!Schema::hasColumn('companies', 'custom_settlement_delay_hours')) {
                $table->integer('custom_settlement_delay_hours')->nullable()->comment('Custom settlement delay for this company');
            }
            if (!Schema::hasColumn('companies', 'custom_settlement_minimum')) {
                $table->decimal('custom_settlement_minimum', 15, 2)->nullable()->comment('Custom minimum settlement amount');
            }
        });

        // Create settlement queue table only if it doesn't exist
    ...
    public function down(): void
    {
        ...
        Schema::table('companies', function (Blueprint $table) {
            $columns = ['custom_settlement_enabled', 'custom_settlement_delay_hours', 'custom_settlement_minimum'];
